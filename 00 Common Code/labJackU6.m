...
classdef labJackU6 < handle
	...
	properties
		...
		% LabJack Exodriver mfilename (for compiled applications)
		Exodriver_mfilename = 'Exodriver';        
		% level of messages 
    ...
    versionStr = 'version 1.3';     % version of this classdef file
    handle = [];                    % The handle for the device
    deviceID = 6;                   % The identifier for the model number of the LabJack (6 = U6)
    
    driverVersion                   % Exodriver version number
...
% 		cmdAINsingle = hex2dec(['00'; 'f8'; '02'; '00'; '00'; '00'; '00'; '00'; '00'; '00']);
        bytesIn=zeros(64,1);  % streaming buffer
        isValidHandle = 0     % handle status flag
    end
	...
	methods
      ...
      if ~libisloaded('liblabjackusb')
        try
          if isdeployed
              loadlibrary(obj.ExodriverLibrary, @Exodriver, 'alias', 'liblabjackusb');
          else
      ...
      if obj.isValidHandle
        ...
        if obj.verbose > 1, fprintf(1,'LJU6/close: no handle to close.\n'); end
			end
        end
...
%             cmdStreamConfig(8)= obj.ResolutionADC;          % Resolution Index (1-8, + 9-12 for U6 Pro)
            cmdStreamConfig(8)= 1;                          % Resolution Index (1-8, + 9-12 for U6 Pro)
        ...
        function errorCode = startStream(obj)
          if obj.isStreaming && obj.verbose > 1
            fprintf(1, 'LJU6/startStream: call to startStream while current streaming\n');
          end
          if ~obj.isStreamConfig && obj.verbose > 1
            fprintf(1,'LJU6/startStream: Error: device not configured for streaming. Use streamConfig.\n');
          else
            obj.usbWrite(obj.cmdStartStream);     % send command
            in = obj.usbRead(4);                  % read response
            errorCode = in(3);                    % VALIDATE RESPONSE CHECKSUMS
            if errorCode == 0
                if obj.verbose > 1, fprintf(1,'LJU6/startStream: Stream started\n'); end
                obj.isStreaming = 1;
            else
                fprintf(1, 'LJU6/startStream: Error %d\n', errorCode);
                obj.isStreaming = 0;
                if errorCode == 48                  % STREAM_IS_ACTIVE
                  fprintf(1, 'LJU6/startStream: Attempting to stop ongoing stream\n');
                  obj.usbWrite(obj.cmdStopStream);  % send command
                  in = obj.usbRead(4);              % read response
                  errorCode = in(3);                % VALIDATE RESPONSE CHECKSUMS
                  if errorCode == 0 || errorCode == 52
                    fprintf(1, 'LJU6/startStream: Stopped ongoing stream\n'); 
                    startStream(obj);
                  else
                    fprintf(1, 'LJU6/startStream: Failed to stop ongoing stream (%d)\n', errorCode);
                  end                
                end
            end     
          end
        ...
        function errorCode = stopStream(obj)
          if obj.isStreamConfig
            ...
            if errorCode == 0 || errorCode == 52
              if obj.verbose > 1
                fprintf('LJU6/stopStream: stream stopped\n'); 
              end
              obj.isStreaming = 0;
            else
        ...
        function [data, errorCode] = getStreamData(obj)
            if obj.isStreamConfig
                ...
                if obj.verbose >= 2, tic; end
                count = length(obj.bytesIn);
                if obj.verbose >= 2
                    fprintf('LJU6/getStreamData:\n');
                end
                if obj.verbose >= 3
                  fprintf('  attempting to read %d bytes from stream\n', count);
                end
                ...
                if obj.verbose >= 3
                    fprintf(1,'  packet %d: %d/%d bytes, %d/%d samples\n', packetNumber, length(obj.bytesIn),...
                          in3, numSamples, length(obj.bytesIn(13:end-2)) / 2);
                end
                if obj.verbose >= 2
                    fprintf(1,'  backlog %.1f%% (error %d)\n', backlog / 2.56, errorCode);
                end
                if obj.verbose >= 2
                  if errorCode == 59
                    fprintf(1, 'LJU6/getStreamData: WARNING: In auto-recovery mode. Data has been lost.\n'); 
                  elseif errorCode ~= 0
                    fprintf(1,'LJU6/getStreamData: error %d on packet %d\n', errorCode, packetNumber);
                  end
                end
                ...
                while backlog > 0
                  ...
                  if obj.verbose >= 3
%                       fprintf(1,'  stream read %d\n', m);
                      fprintf(1,'  packet %d contains %d/%d bytes, %d/%d samples\n', packetNumber, length(obj.bytesIn),...
                          in3, numSamples, length(obj.bytesIn(13:end-2)) / 2);
                  end
                  if obj.verbose >= 2
                    if backlog > maxBacklog
                      fprintf(1, '  read packet %d, backlog %.1f%% (error %d)\n', packetNumber, backlog / 2.56, errorCode);
                      maxBacklog = backlog;
                    end
                  end                    
                  if obj.verbose >= 1
...
%                         fprintf(1,'LJU6/getStreamData: error %d on packet %d\n', errorCode, packetNumber);
                    ...
                    if m >= 255
                        fprintf(1,'LJU6: WARNING: unable to keep up with data stream.\n');
                        break
                    end
                  end
                end
...
%                 end
                ...
                if errorCode == 60
                  ...
                  if obj.verbose > 1 || scansMissed > 0
                    fprintf(1,'LJU6/getStreamData: Auto-recovery: %d scans lost\n', scansMissed); 
                  end
