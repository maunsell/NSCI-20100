...
%     handles.taskData = taskData;                                             % pass to LabJack object
    EOGPlots(handles.lbj, handles.data, [handles.axes1 handles.axes2 handles.axes3 handles.axes4], 0, 0, handles.saccades);
...
handles.ampDur = EOGAmpDur(handles.axes5, handles.data.offsetsDeg);
...
%         'TimerFcn', {@collectData}, 'StartDelay', 0.1); % StartDelay allows other parts of the gui to execute
    ...
    taskTimer = timer('Name', 'TaskTimer', 'ExecutionMode', 'fixedRate',...
        'Period', 0.1, 'UserData', handles.data, 'ErrorFcn', {@timerErrorFcnStop, handles}, 'TimerFcn',...
        {@taskController, handles, handles.lbj, handles.visStim, handles.saccades, handles.ampDur, handles.rtDists...
        [handles.axes1 handles.axes2 handles.axes3 handles.axes4]});
...
function taskController(obj, events, handles, lbj, visStim, saccades, ampDur, rtDists, daqaxes)


lbj = handles.lbj;                                                              % get handle to LabJack
data = obj.UserData;                                                    % data is UserData of LabJack
switch data.taskState
    ...
    case TaskState.taskEndtrial
        [data, startIndex, endIndex] = processSignals(saccades, data);
        EOGPlots(lbj, data, daqaxes, startIndex, endIndex, saccades);
        addAmpDur(ampDur, data.offsetIndex, startIndex, endIndex, data.sampleRateHz);
        plotAmpDur(ampDur);
