...
% O displays an analog data stream from a LabJack U3 or U6
%
% O signals come through analog input channels 0 (AIN0) on the LabJack. Connect LabJack DAC0 to AIN0
...
%% closeO: clean up
function closeO(hObject, eventdata, handles)
...
%     selection = questdlg('Really exit O? Unsaved data will be lost.',...
...
%% initO: initialization
function varargout = initO(hObject, eventdata, handles)                    %#ok<*INUSL>
% initialize application.  We need to set up GUI items  after the GUI has been
% created by after openO function. This method gets called after the GUI is
...
%  get hardware info and do not continue if daq device/drivers unavailable
    ...
    if isempty(lbj.handle)
        ...
        fprintf(1,'Oscilloscope: LabJack Ready\n\n');
    end
    ...
    if errorCode > 0
        fprintf(1,'Oscilloscope: Unable to configure LabJack. Error %d.\n',errorCode);
        return
    end
end

%% openO: just before gui is made visible.
function openO(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to O (see VARARGIN)
    ...
    set(hObject, 'CloseRequestFcn', {@closeO, handles});          	% close function will close LabJack
    handles.lbj = openLabJack();                                    % LJ first, it has no contigencies
    handles.data = OData(handles);                                 % data next
    handles.signals = OSignalProcess(handles);                     % signal processing after data
    handles.plots = OPlots(handles);                              	% plots after handles.data
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    OControlState(handles, 'off', {})
    [fileName, filePath] = uiputfile('*.pdf', 'Save Window Plots as PDF', '~/Desktop/OPlots.pdf');
    if fileName ~= 0
...
%         handles.data.dataState = DataState.dataIdle;            % set data state to idle
        ...
        OControlState(handles, 'off', {handles.startButton})
...
%         profile on;
    ...
    % Stop -- we're already running, so it's a the stop button    
        ...
        set(handles.startButton, 'string', 'Start','backgroundColor', 'green');
        OControlState(handles, 'on', {handles.startButton})
        drawnow;
%         profile viewer
    end
