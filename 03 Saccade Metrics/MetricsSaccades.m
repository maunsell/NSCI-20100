classdef MetricsSaccades < handle
    ...
    methods
      ...
      function [startIndex, endIndex] = processSignals(obj, app, data)
        ...
        [startIndex, endIndex] = obj.findSaccade(data, app, data.posTrace, data.velTrace, data.stepSign, sIndex);
        ...
        [sAvgIndex, eAvgIndex] = obj.findSaccade(data, app, data.posAvg(:, data.offsetIndex), ...
          data.velAvg(:, data.offsetIndex), data.stepSign, length(data.posAvg(:, data.offsetIndex)) / 2);
        if eAvgIndex > sAvgIndex
          data.saccadeDurS(app.absStepIndex) = (eAvgIndex - sAvgIndex) / data.sampleRateHz;
        else
          data.saccadeDurS(app.absStepIndex) = 0;
        end
      end
